...
def get_neighbours(index):
    if debug:
        print(f"get neigbours called... for {index}")
    neighbour_list=[]
    ...
    if debug:
        print(f"neighbour_list at index {index}:   is {neighbour_list}")
...
def render_state(initial):
    ...
    while heap_min:
        ...
        if expanded_nodes==limit:
            ...
            exit()
        if debug:
            print("popped_node: ", popped_node.state)
        ...
        for node in curr_state_list:
            if node==final:
                print(f"Sucess! at depth: {popped_node.f_n+1}")
...
def next_move(neighbour_list, old_state, wrng_mov_idxs, blank_index):
    ...
    for idx in neighbour_list:
        ...
        if debug:    
            print("updated: curr_state: ", curr_state)
